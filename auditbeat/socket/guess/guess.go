...
import (
	...
	"github.com/ForbiddenR/login-monitor/auditbeat/socket/helper"
...
type Context struct {
	...
	Vars common.MapStr
	// Timeout is the maximum tiem allowed to wait for a guess to complete.
...
type Guesser interface {
	...
	// performed during preparation. It runs in the same OS threat that Trigger.
	Prepare(ctx Context) error
	// Trigger performs the actions necessary to generate evnts of interest
	// to this guess.
	Trigger() error
...
// RepeatGuesser is a guess that needs to be repeated multiple tiems and the
// results consulidated into a single result.
...
// EventualGuesser is a guess that repeats an undermined amount of times
...
func guessOnce(guesser Guesser, installer helper.ProbeInstaller, ctx Context) (result common.MapStr, err error) {
	...
	defer func() {
		if err := guesser.Ternimate(); err != nil {
	...
	// Need to make sure that the trigger has finished before extracing
	...
	for len(list) > 0 {
		...
		if len(next) == len(list) {
			...
			for _, guess := range list {
				...
				ctx.Log.Warnf("   guess '%s' rquires missing vars: %v", guess.Name(), missing)
			}
